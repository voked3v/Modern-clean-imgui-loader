...
{
	if (!g_pProcess->is_process_running(proccessname))
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Process not running"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		exit(0);
	}

	g_pProcess->dwProcessId = g_pProcess->GetProcessID(proccessname);
	//MessageBox(NULL, xorstr_("Process found"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);

	if (!g_pProcess->dwProcessId)
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Failed to get process id"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		exit(0);
	}

	g_pProcess->hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, g_pProcess->dwProcessId);
	if (!g_pProcess->hProcess)
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("OpenProcess failed"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		exit(0);
	}
	...
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Opening the file failed"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		file.close();
		CloseHandle(g_pProcess->hProcess);
		exit(0);
	}
	...
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Invalid file size"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		file.close();
		CloseHandle(g_pProcess->hProcess);
		exit(0);
	}
	...
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Can't allocate dll file"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		file.close();
		CloseHandle(g_pProcess->hProcess);
		exit(0);
	}
	...
	if (!ManualMapDll(g_pProcess->hProcess, pSrcData, FileSize))
	{
		std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
		MessageBox(NULL, xorstr_("Injection failed"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
		delete[] pSrcData;
		CloseHandle(g_pProcess->hProcess);
		exit(0);
	}
	...
	CloseHandle(g_pProcess->hProcess);
	std::remove((util::GetExeDirectory() + xorstr_("\\") + dll_path).c_str());
	MessageBox(NULL, xorstr_("Injection successful, please start the game now"), xorstr_("ExentriC - Information"), MB_OK | MB_ICONINFORMATION);
	return 0;
}

MANUAL_MAPPING_DATA* g_manual_map = new MANUAL_MAPPING_DATA;