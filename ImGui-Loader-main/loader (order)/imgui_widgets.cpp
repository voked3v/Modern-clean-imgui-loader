...
#include "../loader (order)/examples/example_win32_directx11/colors.h"
...
bool ImGui::SubTabs(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (it_anim->second.rect_size > 0.5f) ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(5, label_size.y + 6), bb.Max + ImVec2(-label_size.x - 2 + it_anim->second.rect_size, 2), GetColorU32(c::rect_active), 30);
...
bool ImGui::Tabs(bool selected, const char* ico_label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(ico_label);
    const ImVec2 label_size = CalcTextSize(ico_label, NULL, true);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? c::text_tabs : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);

    PushFont(ico);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, ico_label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleColor();
    PopFont();

    IMGUI_TEST_ENGINE_ITEM_INFO(id, ico_label, g.LastItemData.StatusFlags);
    return pressed;
}
...
bool ImGui::CButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, cbutton_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::text_tabs : hovered ? c::text_tabs : c::text, g.IO.DeltaTime * 6.f);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, IsItemActive() ? c::button_active : hovered ? c::button_hov : c::button, g.IO.DeltaTime * 6.f);
...
bool ImGui::InjectButton(const char* id_label, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, IsItemActive() ? c::button_active : hovered ? c::ibutton_hov : c::ibutton, g.IO.DeltaTime * 6.f);

    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background_color), true, 8.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, GetColorU32(c::scroll_bg), window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(1, 2), grab_rect.Max + ImVec2(1, 2), GetColorU32(c::scroll_rect), style.ScrollbarRounding);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& size_rect, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    ImGui::SetCursorPos(GetCursorPos() + ImVec2(3.2f, 0));

    const ImVec2 padding = g.Style.FramePadding;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + size_rect + padding * 0.0f);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size + padding * 0.0f);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, image_state() });
        it_anim = anim.find(id);
    }

   // it_anim->second.arc_color = ImLerp(it_anim->second.arc_color, IsItemActive() ? c::picture_active : hovered ? c::picture_hov : c::picture, g.IO.DeltaTime * 6.f);

    RenderFrame(rect.Min, rect.Max + ImVec2(0, 0), GetColorU32(c::picture_background), true, 10.f);
...
void ImGui::StatusClient(ImTextureID texture_id, const ImVec2& size, const ImVec2& size_rect, const char* game, bool undetected, const char* version, const char* update_date, const char* price) {

    ImGuiWindow* window = GetCurrentWindow();
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;

    ImGui::ImageButton(texture_id, size, size_rect);

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos);

    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 170, rect.Max.y - 145), GetColorU32(c::text_hov), game);
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 170, rect.Max.y - 115), GetColorU32(c::text_hov), version);
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 170, rect.Max.y - 85), GetColorU32(c::text_hov), update_date);
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 170, rect.Max.y - 55), GetColorU32(undetected ? c::undetected : c::detected), undetected ? "Status: Undetected" : "Status: Detected");

    window->DrawList->AddRectFilled(rect.Min + ImVec2(425, -55), rect.Max + ImVec2(480, -28), GetColorU32(c::price_background), 5.f);

    RenderTextClipped(rect.Min + ImVec2(426, -55), rect.Max + ImVec2(481, -28), price, NULL, NULL, ImVec2(0.5f, 0.5f));
}

void ImGui::SupportList(ImTextureID texture_id, const ImVec2& size, const ImVec2& size_rect, const char* name, ImU32 name_color, const char* status, ImU32 status_color) {
    ...
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 100, rect.Max.y - 75), name_color, name);
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 100, rect.Max.y - 48), status_color, status);

}

void ImGui::InjectProduct(ImTextureID texture_id, const ImVec2& size, const ImVec2& size_rect, const char* name_product, ImU32 product_color, const char* active_until, ImU32 until_color) {
    ...
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 100, rect.Max.y - 75), product_color, name_product);
    ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 100, rect.Max.y - 48), until_color, active_until);

    ImGui::SetCursorPos(GetCursorPos() + ImVec2(331, -69.5f));
    if (ImGui::InjectButton(name_product, "Inject", ImVec2(150, 40)));

    ImGui::SetCursorPos(GetCursorPos() + ImVec2(0, 18.f));
}

bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& size_rect, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(window->GetID(str_id), user_texture_id, size, size_rect, uv0, uv1, bg_col, tint_col);
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& size_rect, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    bool ret = ImageButtonEx(id, user_texture_id, size, size_rect, uv0, uv1, bg_col, tint_col);
    if (frame_padding >= 0)
        PopStyleVar();
    return ret;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.f_color = ImLerp(it_anim->second.f_color, *v ? c::checkbox_active : hovered ? c::checkbox_hov : c::checkbox, g.IO.DeltaTime * 6.f);
    ...
    ImGui::GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox_background), style.FrameRounding, ImDrawFlags_None, 2.f);
    ImGui::GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(5, 5), check_bb.Max - ImVec2(5, 5), GetColorU32(it_anim->second.f_color), style.FrameRounding);

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 2.5f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 10.0f : label_size.y) + style.FramePadding.y * 3.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        ...
        it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, IsItemActive() ? c::rect_move : hovered ? c::rect : c::rect, g.IO.DeltaTime * 4.f);
        it_anim->second.move_rect = ImLerp(it_anim->second.move_rect, IsItemActive() ? label_size.x : 0.f, g.IO.DeltaTime * 6.f);

        ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(4, label_size.y + 7), frame_bb.Max, GetColorU32(it_anim->second.rect_color), 30.f);

     //   ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(4, label_size.y + 6), frame_bb.Max + ImVec2(-frame_bb.Max.x + it_anim->second.move_rect, 0-), GetColorU32(it_anim->second.rect_color), 30.f);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::delete_text), 3.f);
                }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 0, 6), frame_bb.Min + ImVec2(it_anim->second.value + 1, 23), GetColorU32(ImGuiCol_Text));
    ...
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x - CalcItemWidth(), frame_bb.Min.y + style.FramePadding.y - 25), label);
